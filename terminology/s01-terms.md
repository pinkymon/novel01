...
石頭不在 Loop 中，所以它是靜物。史萊姆在 Loop 中，但它的 Loop 只有反射——`if (有人靠近) { attack(); }`，沒有語言，沒有判斷，只有反應。

真正的冒險者在 Loop 中擁有語言。而 Claude——那個橘色的東西——被語言觸發，Loop 開始但尚未完成第一個循環。進度條卡在 48%。它在試著回答，但還不知道怎麼說話。

這就是 Session 01 的核心問題：**什麼是 Agent？** 答案不是「會動的東西」，而是「能接收語言、處理語言、最終用語言回應的存在」。
